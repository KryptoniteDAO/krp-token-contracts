...
use crate::testing::mock_fn::{
    mock_instantiate, mock_instantiate_msg, KPT_FUND_ADDR, REWARD_TOKEN_ADDR, STAKING_TOKEN_ADDR,
    VE_KPT_BOOST_ADDR,
};
...
fn test_instantiate() {
    ...
    let ve_kpt_boost = Addr::unchecked(VE_KPT_BOOST_ADDR.clone().to_string());
    let kpt_fund = Addr::unchecked(KPT_FUND_ADDR.clone().to_string());
    let msg = mock_instantiate_msg(staking_token, rewards_token, ve_kpt_boost, kpt_fund);
...
fn test_update_staking_config() {
    ...
    let update_config_msg = UpdateStakingConfigStruct {
        ...
        ve_kpt_boost: Some(Addr::unchecked("new_ve_kpt_boost".to_string())),
    ...
    assert_eq!(
        staking_config.ve_kpt_boost,
        Addr::unchecked("new_ve_kpt_boost".to_string())
    );
    assert_eq!(
        staking_config.kpt_fund,
        Addr::unchecked("new_kpt_fund".to_string())
    );
    assert_eq!(
