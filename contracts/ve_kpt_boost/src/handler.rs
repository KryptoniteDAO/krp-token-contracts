...
use crate::state::{read_boost_config, read_user_lock_status, store_boost_config, store_user_lock_status, VeKptLockSetting};
...
pub fn add_lock_setting(deps: DepsMut, info: MessageInfo, duration: Uint128, mining_boost: Uint128) -> StdResult<Response> {
    ...
    ve_kpt_lock_settings.push(VeKptLockSetting {
        duration,
        mining_boost,
    });
...
pub fn set_lock_status(deps: DepsMut, env: Env, info: MessageInfo, index: usize) -> StdResult<Response> {
    ...
    let setting: VeKptLockSetting = config.ve_kpt_lock_settings[index].clone();
